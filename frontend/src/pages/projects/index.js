...
const delete_project_dialog = new MDCDialog(
  document.querySelector("#delete-project-dialog")
);
var deleteProjectDialogEventListener;

document
  .getElementById("import-project-input")
  .addEventListener("change", importProjectFilesInputChanged);
document
  .getElementById("orderby-menu-open-button")
  .addEventListener("click", openOrderByMenuClicked);
document
  .getElementById("new-project-button")
  .addEventListener("click", newProjectButtonClicked);
document
  .getElementById(`orderby-menu-name-asc`)
  .addEventListener("click", orderBy.bind(null, "name", "asc"));
document
  .getElementById(`orderby-menu-name-desc`)
  .addEventListener("click", orderBy.bind(null, "name", "desc"));
document
  .getElementById(`orderby-menu-created-asc`)
  .addEventListener("click", orderBy.bind(null, "created", "asc"));
document
  .getElementById(`orderby-menu-created-desc`)
  .addEventListener("click", orderBy.bind(null, "created", "desc"));
document
  .getElementById(`orderby-menu-edited-asc`)
  .addEventListener("click", orderBy.bind(null, "edited", "asc"));
document
  .getElementById(`orderby-menu-edited-desc`)
  .addEventListener("click", orderBy.bind(null, "edited", "desc"));
document
  .getElementById(`button-pagination-first`)
  .addEventListener("click", goToPage.bind(null, "first"));
document
  .getElementById(`button-pagination-last`)
  .addEventListener("click", goToPage.bind(null, "last"));
...
async function getProjects(
  ...
  if (response.status == 200) {
    ...
    data.forEach((project) => {
      ...
      const num_annotated = countAnnotatedImages(
        project.images,
        existing_anotations
      );
...
function makePagination() {
  ...
  if (num_pages > 1) {
    const pagination = document
      .getElementById("pagination-projects")
      .querySelector(".pagination-main");
    ...
    for (var page_num = 1; page_num <= num_pages; page_num++) {
      ...
      document
        .getElementById(`button-pagination-${page_num}`)
        .addEventListener("click", goToPage.bind(null, page_num));
    }
  } else {
...
function updatePagination(page_num) {
  ...
  const dots_left = document
    .getElementById("pagination-projects")
    .querySelector(".pagination-dots-left");
  const dots_right = document
    .getElementById("pagination-projects")
    .querySelector(".pagination-dots-right");
...
function highlightActivePage(page_num) {
  ...
  pagination_buttons
    .querySelector(`#button-pagination-${page_num}`)
    .classList.add("button-pagination-active");
}
...
function addProjectToProjectList(project, num_images, num_annotated) {
  ...
  document
    .getElementById("projects-list-menu-list")
    .appendChild(htmlToElements(html_menu_item));

  document
    .getElementById(`projects-list-item-${project.id}`)
    .addEventListener("click", annotateProjectClicked.bind(null, project.id));
  document
    .getElementById(`projects-list-item-menu-button-${project.id}`)
    .addEventListener("click", openMenu.bind(null, project.id));
  document
    .getElementById(`projects-list-menu-annotate-${project.id}`)
    .addEventListener("click", annotateProjectClicked.bind(null, project.id));
  document
    .getElementById(`projects-list-menu-setup-${project.id}`)
    .addEventListener("click", setupProjectClicked.bind(null, project.id));
  document
    .getElementById(`projects-list-menu-export-${project.id}`)
    .addEventListener("click", exportProjectClicked.bind(null, project.id));
  document
    .getElementById(`projects-list-menu-delete-${project.id}`)
    .addEventListener("click", deleteProjectClicked.bind(null, project.id));
  project_list_menus[project.id] = new MDCMenu(
    document.querySelector(`#projects-list-menu-${project.id}`)
  );
  init_mui_elements();
}
...
async function annotateProjectClicked(project_id) {
  ...
  if (response.status == 200) {
    ...
    throw new Error(
      `Failed to get url for annotating project with id ${project_id}`
    );
  }
}
...
function init_mui_elements() {
  const listItemRipples = [].map.call(
    document.querySelectorAll(".mdc-list-item"),
    function (element) {
