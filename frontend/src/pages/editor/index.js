...
const imagesSelectionList = new MDCList(
  document.getElementById("images-selection-list")
);
...
async function editor(project_id) {
  ...
  document
    .getElementById("export-project-button")
    .addEventListener("click", exportProjectClicked.bind(null, project_id));
  document
  ...
  function addImageToImagesList(image) {
    const html_list_item = `
          <li id="images-selection-list-${image.id}" data-image-id="${
      image.id
    }" data-image-name="${
      image.name
    }" class="mdc-deprecated-list-item" role="option">
            <span class="mdc-deprecated-list-item__ripple"></span>
            <span class="mdc-deprecated-list-item__text">${splitext(
              image.name
            ).slice(0, 8)}</span>
    ...
    document
      .getElementById("images-selection-list")
      .appendChild(htmlToElements(html_list_item));
  }

  function setImageAnnotationStatus(image_id, annotated) {
    const listElement = document.getElementById(
      `images-selection-list-${image_id}`
    );
  ...
  function images_selection_changed(event) {
    ...
    var g_element = svg.append("g").attr("id", "g_element");

    var image_element = svg.select("#g_element").append("image");
    ...
    img.onload = function () {
      ...
      var initial_transform = d3.zoomIdentity.translate(
        img_offset_x,
        img_offset_y
      );
    ...
    svg.select("#g_element").append("g").attr("id", "g_auxobjects");
    svg.select("#g_element").append("g").attr("id", "g_pv_modules");
    svg.select("#g_element").append("g").attr("id", "g_corners");
  ...
  document
    .getElementById("images-selection-list")
    .addEventListener("MDCList:selectionChange", images_selection_changed);
  ...
  svg.on("mousemove", (event) => {
    ...
    if (!d3.select("#g_element").empty()) {
      var [img_offset_x, img_offset_y, img_scale] =
        getTranslationScale(g_element);
      ...
      // draw cursor
      svg.select("#cursor").attr("cx", img_pos_x).attr("cy", img_pos_y);

      // make center connection lines invisible
      for (var i = 0; i < 4; i++) {
        svg.select("#center-line-" + [i]).style("stroke-opacity", 0);
      }
      ...
      var findKNearest = function (point, corners, k) {
        ...
        for (var i = 0; i < corners.length; i++) {
          var dist =
            Math.pow(corners[i].x - point.x, 2) +
            Math.pow(corners[i].y - point.y, 2);
          distances.push({
            dist: dist,
            corner: corners[i],
          });
        }
        var distances_sorted = distances.sort(function (a, b) {
      ...
      if (corners.length > 0 && marker_mode == "center") {
        neighbour_corners = findKNearest(
          { x: img_pos_x, y: img_pos_y },
          corners,
          4
        );
  ...
  svg.on("mouseover", function () {
    ...
    if (marker_mode == "center") {
      for (var i = 0; i < 4; i++) {
        svg.select("#center-line-" + [i]).style("stroke-opacity", 1);
      }
    }
  });
  ...
  var button_drawing_click = function (e) {
    ...
    if (btn.id == "btn-add-corner") {
      ...
      btn.classList.add("btn-active");
      svg.select("#cursor").style("fill-opacity", 0).style("stroke", null);
  ...
  svg.on("click", function () {
    if (marker_mode == "corner") {
      ...
      if (corners.length < 4) {
        alert(
          "Before you can create a grid cell you need to place at least four corner markers first."
        );
      } else {
      ...
      if (temp_auxline_pts.length == 2) {
        ...
        console.log(
          "Placed auxiliary line from (",
          temp_auxline_pts[0].x,
          ",",
          temp_auxline_pts[0].y,
          ") to (",
          temp_auxline_pts[1].x,
          ",",
          temp_auxline_pts[1].y,
          ")"
        );
        temp_auxline_pts = [];
        selected_image_changed();
  ...
  var order_corners_ccw = function (corners) {
    var corners_sorted = corners.sort(function (a, b) {
      return a.x > b.x ? 1 : -1;
    });
    var corners_left = corners_sorted.slice(0, 2);
    var corners_right = corners_sorted.slice(2, 4);
    var corners_left_sorted = corners_left.sort(function (a, b) {
      return a.y > b.y ? 1 : -1;
    });
    var tl = corners_left_sorted[0];
    var bl = corners_left_sorted[1];
    var corners_right_sorted = corners_right.sort(function (a, b) {
      return a.y > b.y ? 1 : -1;
    });
  ...
  function compute_intersections() {
    ...
    function intersect_paths(path0, path1) {
      ...
      for (i in overlays.points) {
        if (
          overlays.points[i] instanceof Vector2D ||
          overlays.points[i] instanceof Point2D
        ) {
          intersects.push({
            x: overlays.points[i].x,
            y: overlays.points[i].y,
          });
        }
      }
    ...
    function get_diff(prev_intersects, intersects) {
      ...
      for (var i = 0; i < intersects.length; i++) {
        if (
          prev_intersects.some(function (d) {
            return d.x == intersects[i].x && d.y == intersects[i].y;
          })
        ) {
      ...
      for (var i = 0; i < prev_intersects.length; i++) {
        if (
          !intersects.some(function (d) {
            return d.x == prev_intersects[i].x && d.y == prev_intersects[i].y;
          })
        ) {
    ...
    for (var k = corners.length - 1; k >= 0; k--) {
      if (
        removed.some(function (d) {
          return d.x == corners[k].x && d.y == corners[k].y;
        })
      ) {
    ...
    if (marker_mode == "center_manual") {
      if (corners.length < 4) {
        ...
        if (neighbours_manual_selection.length == 4) {
          neighbours_manual_selection = order_corners_ccw(
            neighbours_manual_selection
          );
          var center_point = compute_center_point(neighbours_manual_selection);
          // store the new pv module
          pv_modules.push({
            corners: neighbours_manual_selection,
            center: center_point,
            id: uuidv4(),
            truncated: false,
          });
  ...
  function generate_auxcurve_data(curve_points) {
    var line_generator = d3.line().curve(d3.curveCatmullRom.alpha(1));
  ...
  function auxcurve_dragged(event, d) {
    ...
    for (var i = 0; i < auxcurves.length; i++) {
      if (auxcurves[i].id == element_id) {
        ...
        d3.select(this.parentNode)
          .select("path")
          .attr("d", generate_auxcurve_data(auxcurves[i].points));
      }
    }
    // update line point
      ...
      for (var i = corners.length - 1; i >= 0; i--) {
        if (corners[i].id == element_id) {
          ...
          for (var j = prev_intersections.length - 1; j >= 0; j--) {
            if (
              prev_intersections[j].x == corners[i].x &&
              prev_intersections[j].y == corners[i].y
            ) {
  ...
  var draw = function () {
    // auxiliary lines
    var svg_auxlines = svg
      .select("#g_auxobjects")
      .selectAll(".auxline")
      .data(auxlines, (d) => d["id"]);
    var svg_auxlines_enter = svg_auxlines
      .enter()
      .append("g")
      .attr("id", function (d) {
        return d.id;
    ...
    var svg_auxlines_exit = svg_auxlines.exit().each(function () {
      d3.select(this.parentNode).remove();
    });

    // auxiliary curves
    var svg_auxcurves = svg
      .select("#g_auxobjects")
      .selectAll(".auxcurve")
      .data(auxcurves, (d) => d["id"]);
    var svg_auxcurves_enter = svg_auxcurves
      .enter()
      .append("g")
      .attr("id", function (d) {
        return d.id;
    ...
    var svg_auxcurves_exit = svg_auxcurves.exit().each(function () {
      d3.select(this.parentNode).remove();
    });
