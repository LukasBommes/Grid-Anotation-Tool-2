...
async function getAnnotationIds() {
  // async function getAnnotationIds() leads to weird error because it overwrites above definition
...
async function setupProjectClicked(project_id) {
  ...
  if (response.status == 200) {
    ...
    throw new Error(
      `Failed to get url for setting up project with id ${project_id}`
    );
  }
}
