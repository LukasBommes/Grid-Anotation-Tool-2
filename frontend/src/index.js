...
document
  .getElementById("app-bar-home-button")
  .addEventListener("click", homeButtonClicked);
document
  .getElementById("app-bar-login-button")
  .addEventListener("click", loginButtonClicked);

const iconButtonRipples = [].map.call(
  document.querySelectorAll(".app-bar-icon-button"),
  function (element) {
...
async function setLoginButton() {
  const login_button_label = document.getElementById(
    "app-bar-login-button-label"
  );
  const login_button_icon = document.getElementById(
    "app-bar-login-button-icon"
  );
  const loggedIn = await userIsLoggedIn();
  if (loggedIn) {
