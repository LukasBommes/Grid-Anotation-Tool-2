...
def get_image_(image_id, db, current_user):
    ...
    if not db_image:
        raise HTTPException(
            status_code=404, detail=f"Image with id {db_image} not found"
        )
    return db_image
...
def create_router(settings):
    ...
    def get_images(
        ...
        db_images_query = db.query(models.Image).filter(
            and_(
                models.Image.username == current_user.username,
                models.Image.project_id == project_id,
            )
        )
        response.headers["X-Total-Count"] = json.dumps(db_images_query.count())
        ordered_column = getattr(getattr(models.Image, orderby), orderdir)()
        db_images = (
            db_images_query.order_by(ordered_column).offset(skip).limit(limit).all()
        )
        return db_images
    ...
    def get_image_file(
        ...
        filepath = os.path.join(
            settings.MEDIA_ROOT, f"project_{db_image.project_id}", db_image.name
        )
        return filepath
        ...
        if not db_project:
            ...
            for file in files:
                ...
                if not filetype.match(contents, matchers=filetype.image_matchers):
                    raise HTTPException(
                        status_code=422, detail=f"Uploaded file is not a valid image."
                    )
                ...
                db_image = models.Image(
                    name=filename, project_id=project_id, username=current_user.username
                )
                db.add(db_image)
                db.commit()
                ...
                db_annotation = models.Annotation(
                    id=db_image.id, username=current_user.username
                )
                ...
                db.refresh(
                    db_image
                )  # should probably instead go right after commit of db_image
