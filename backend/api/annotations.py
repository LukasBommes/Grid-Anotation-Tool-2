...
def create_router(settings):
    ...
    def get_annotation_ids(
        ...
        db_annotations = (
            ...
            .filter(models.Annotation.username == current_user.username)
            .all()
        )
    ...
    @router.get(
        "/annotation/{image_id}", response_model=schemas.Annotation, status_code=200
    )
    def get_annotation(
        image_id: int,
        db: Session = Depends(get_db),
        ...
        if not db_annotation:
            raise HTTPException(
                status_code=404, detail=f"Annotation with id {image_id} not found"
            )
